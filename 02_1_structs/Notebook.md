1. [クラスと構造体（struct/class）ノート](#クラスと構造体structclassノート)
   ...
   4. [初期化：集成体初期化（aggregate）とコンストラクタ](#初期化集成体初期化aggregateとコンストラクタ)
      ...
      2. [丸括弧と波括弧による初期化の違い](#丸括弧と波括弧による初期化の違い)
         1. [`{}`が安全と言われる理由](#が安全と言われる理由)
         2. [`explicit`との関係](#explicitとの関係)
         3. [空の`{}`と注意点](#空のと注意点)
         4. [`initializer_list`がある型では意味が変わることがある](#initializer_listがある型では意味が変わることがある)
      3. [class の典型：不変条件（invariant）を守る](#class-の典型不変条件invariantを守る)
...
C++では、オブジェクトを作るときに `()` と `{}` の2種類の書き方がよく登場します。
...
`()`を使う書き方は**直接初期化**、`{}`を使う書き方は**直接リスト初期化**と呼ばれます。
この`Point`は集成体なので、C++20ではどちらもメンバの宣言順に`x`と`y`を初期化します。
このプロジェクトはC++20を使っているため、`Point p(3.0, 4.0);`も有効です。

一方、コンストラクタを持つ`class`では、次のようにどちらの書き方もコンストラクタを呼び出します。
...
- `BankAccount account_with_parentheses(100);`
  - `BankAccount(int)`を直接初期化で呼び出す
- `BankAccount account_with_braces{100};`
  - `BankAccount(int)`を直接リスト初期化で呼び出す

この2つは、現在の`BankAccount`では同じ値になります。実際の動作は[main.cpp](main.cpp)で確認できます。

#### `{}`が安全と言われる理由

波括弧による初期化は、値が小さい型へ変換される**縮小変換**を原則として許可しません。

```cpp
int a(3.14);  // 許可される場合がある。小数部分が失われて3になる
// int b{3.14}; // コンパイルエラー: doubleからintへの縮小変換
```

`BankAccount`のコンストラクタは`int`を受け取るため、次のコードもエラーになります。

```cpp
// BankAccount invalid{100.5};
```

意図しないデータ損失を早く発見できるため、特別な理由がなければ`{}`を使うスタイルがよく採用されます。

#### `explicit`との関係

`BankAccount`のコンストラクタには`explicit`が付いています。
`explicit`は、代入形式による暗黙変換を禁止しますが、`()`や`{}`による明示的な初期化は妨げません。

```cpp
BankAccount a{100}; // OK: 明示的にBankAccountを作っている
// BankAccount b = 100; // エラー: intからの暗黙変換は禁止
```

#### 空の`{}`と注意点

```cpp
Point origin{}; // xとyが0で初期化される
```

集成体の空の`{}`は、メンバを値初期化するため、数値メンバは0になります。

```cpp
Point origin(); // オブジェクトではなく、関数宣言として解釈される
```

引数のないオブジェクトを`()`で書くと、関数宣言に見えてしまうことがあります。
この問題は**most vexing parse**と呼ばれ、空のオブジェクトを作るときは`{}`を使うと避けられます。

#### `initializer_list`がある型では意味が変わることがある

クラスが`initializer_list`用のコンストラクタを持っている場合、`{}`はそれを優先して選びます。
例えば`std::vector`では、次の2つは意味が異なります。

```cpp
std::vector<int> values_by_count(3, 1); // 1を3個: {1, 1, 1}
std::vector<int> values_by_list{3, 1};  // 要素3と1: {3, 1}
```

つまり、`{}`は常に`()`と同じではありません。
基本的には縮小変換を防ぐ`{}`を使い、`initializer_list`との意味の違いを意識する場面では、型のコンストラクタを確認します。
